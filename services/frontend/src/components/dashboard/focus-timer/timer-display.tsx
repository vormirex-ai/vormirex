...
export function TimerCard({ activeTask }: any) {
  ...
  const progress =  ((currentTab.time - seconds) / currentTab.time) * 100;
  ...
  const handleSessionComplete = async () => {
    try {
      ...
      const sessionType = currentTab.label === "Focus" ? "focus" : currentTab.label === "Short Break"
            ? "short-break" : "long-break";

      const res = await createFocusSession({
        taskId: sessionType === "focus" ? activeTask?._id : undefined,
        durationMinutes: currentTab.time / 60,
        type: sessionType,
      }).unwrap();

      toast.success(`${currentTab.label} completed 🎉`);
    ...
    <div className="rounded-2xl sm:rounded-3xl custom-surface p-2 md:p-4 sm:p-6 shadow-lg">
      <div className="grid grid-cols-3 overflow-hidden rounded-xl sm:rounded-2xl border border-primary dark:border-white/10 p-1 bg-card dark:bg-[#154249]">
        {tabs.map((tab, index) => (
          ...
          >
            ...
            <div className="text-[9px] sm:text-xs">{tab.sub}</div>
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col items-center pt-8 pb-4 sm:pt-14">
        ...
        <div className="mt-6 sm:mt-10 flex items-center gap-3 sm:gap-6">
          <button
            onClick={resetTimer}
            className="flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-gray-200 dark:bg-white/5"
          >
          ...
          <button
            onClick={toggleTimer}
            className="flex h-16 w-16 sm:h-20 sm:w-20 items-center justify-center rounded-full bg-primary-gradient"
          >
            {isRunning ? (
