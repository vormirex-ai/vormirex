...
const initialState: UiState = {
  ...
  fontSize: "medium",
  compactSidebar: JSON.parse(
    localStorage.getItem("compactSidebar") || "false"
  ),
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    setUiPreferences: (  state,  action: PayloadAction<Partial<UiState>>) => {
      ...
      if (action.payload.compactSidebar !== undefined) {
        state.compactSidebar = action.payload.compactSidebar;
        localStorage.setItem(
          "compactSidebar",
          JSON.stringify(action.payload.compactSidebar)
        );
      }

      if (action.payload.reducedAnimations !== undefined) {
        state.reducedAnimations =
          action.payload.reducedAnimations;
      }
